...
struct ffm_problem
{
    ffm_int n;   // max of number of feature id + 1.. feature id space.
    ffm_long l;   // num of training examples
    ffm_int m;   // number of fields + 1 (zero dimension is for
                 // standard fm).
    ffm_node *X;    // all feature nodes, length = nnz (number of
                    // nonzero features) in the training set.
    ffm_long *P;    // starting offsets feature nodes for each
                    // example, length = l + 1
    ffm_float *Y;   // label for each example, length = l

    ffm_int num_groups;
    ffm_long* group_offsets;
};
