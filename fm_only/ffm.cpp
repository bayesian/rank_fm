...
#endif
        for(ffm_long ii = 0; ii < (ffm_long)order.size(); ii++)
        {
            ffm_long i = order[ii];
...
ffm_problem* ffm_read_problem(char const *path)
{
    ...
    prob->num_groups = ng;
    prob->group_offsets = new ffm_long[ng];
    ...
    for(ffm_long i = 0; fgets(line, kMaxLineSize, f) != nullptr; i++)
    {
...
{
    vector<ffm_long> order(tr->l);
    for(ffm_long i = 0; i < tr->l; i++)
        order[i] = i;
    ...
    ffm_parameter param)
{
    ...
    {
        ffm_long begin = fold*nr_instance_per_fold;
        ffm_long end = min(begin + nr_instance_per_fold, prob->l);

        vector<ffm_long> order1;
        for(ffm_long i = 0; i < begin; i++)
            order1.push_back(order[i]);
        for(ffm_long i = end; i < prob->l; i++)
            order1.push_back(order[i]);
...
#endif
        for(ffm_long ii = begin; ii < end; ii++)
        {
            ffm_long i = order[ii];
